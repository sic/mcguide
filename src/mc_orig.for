...
*       generator used is "g05dae" from the NAG library. It uses a seed
*       provided by "g05cce" which ensures a non-repeteable sequence.
...
*       Extra definitions for VAX
     ...
     *       ftranse(100), delole(100), avrefe(100)
        INTEGER*2 IGEOM(100,100), IREFLC(100,100), NOJOB
     ...
     $               GAMMB(100),GAMMA(100)
        ...
        COMMON/GEOM/NSURF,NREG,IGEOM
        COMMON/GEOM/A(100),B(100),C(100),D(100),E(100),F(100),G(100),
     $              P(100),Q(100),R(100)
        COMMON/TES/IN
        LOGICAL IN
        EXTERNAL TAY
...
12      CONTINUE
        ...
        IF(IREFLC(IREG,ISURF).EQ.1) GO TO 10
...
217     FORMAT(3F20.10)
219     FORMAT(/,' MCGUIDE (ORIGINAL) : MK 4.2  15/9/80')
...
C ALONG THE DIRECTION VX,VY,VZ FROM X,Y,Z. THE EXIT SURFACE IS
C RETURNED IN IN ISURF.
      INTEGER*2 IGEOM(100,100)
...
*       Tay(x) calculates the intensity at the moderator for each wavelength.
        ...
        real*8 ache, mass, boltz, temp, rho
        real*8 thermalwave, thermalwavesq, thermalwavecube
        real*8 c1, c2, c3, c4, arg1, expw1w2, arg2num, arg2, expw4w5
        real*8 wavsq, expthermal, arg3, c1log
        real*8 w1, w2, w3, w4, w5, phi0, alpha, wav, x
        real*8 phiepi, phimax, phifer, scale, phi
        real*8 phifer1, phifer2, phifer3, scalerho
        real*8 scale3, scale4, scale5, s4, s5, scalew3, scalew2, s2
