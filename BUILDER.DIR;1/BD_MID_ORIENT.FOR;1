...
*	Including possibility of bender orientation about the point
*	midway between the preceeding and next guide sections.
*	This bender is based on a the actual physical shape.	
	...
	COMMON /BEND/SURFLAG,NODIV,INT_COEF,POL_EFF,GAMMB_BEND,ORIENT
     ...
     $		       X2,Y2,THETA2,REFL_ANSWER,x2_norm,y2_norm
     $		       x1_norm, y1_norm,X_MID,Y_MID
         DATA YES/'Y'/
...
*	alpha=-alpha
	...
	X2_norm = X1 + STRAIGHT*DSIN(THETA)
        Y2_norm = Y1 + STRAIGHT*DCOS(THETA)
        
	PRINT*,'X1: ',X1,' Y1: ',Y1
   	PRINT*,'X2: ',X2,' Y2: ',Y2
   
*	Calculates mid point of bender

	X_MID = X1 + (STRAIGHT/2)*DSIN(THETA)
        Y_MID = Y1 + (STRAIGHT/2)*DCOS(THETA)
                         
*	Calculates the "new" beginning and end of the bender

	THETA = THETA + ORIENT

        X2 = X_MID + (STRAIGHT/2)*DSIN(THETA)
        Y2 = Y_MID + (STRAIGHT/2)*DCOS(THETA)

        X1 = X_MID - (STRAIGHT/2)*DSIN(THETA)
        Y1 = Y_MID - (STRAIGHT/2)*DCOS(THETA)
...
*****************************
	PRINT*,'LENGTH1 =',LENGTH1,' BEFORE ANY INTERNAL CALCS'
...
*	Sample plane with respect to exit
	...
	PRINT*,'KNT+1 =',SURFLAG(KNT+1)
	PRINT*,'KNT+3 =',SURFLAG(KNT+3)
	PRINT*,'KNT+6 =',SURFLAG(KNT+6)
	PRINT*,'KNT+7 =',SURFLAG(KNT+7)
	PRINT*,'KNT+8 =',SURFLAG(KNT+8)
